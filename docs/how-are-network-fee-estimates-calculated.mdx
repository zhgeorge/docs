...
description: "zerohash provides an estimated network fee endpoint that returns approximate withdrawal costs based on current blockchain conditions. Fee estimates are available in both PROD and CERT environments.…"
---

zerohash provides an [estimated network fee](https://docs.zerohash.com/reference-link/get_withdrawals-estimate-network-fee) endpoint that returns approximate withdrawal costs based on current blockchain conditions. Fee estimates are available in both PROD and CERT environments.
