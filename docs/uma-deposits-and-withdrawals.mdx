---
title: "UMA Deposits and Withdrawals Integration Guide"
description: "Deposits and Withdrawals using the UMA (LNURL-p) standard on the Lightning Network including generating and sending to invoices."
---
...
<Info title="Note: The Zero Hash UMA VASP linked to UMA.me will use the following default beginning 1/10/25:">
...
| When any of the following query params is not sent: - umaVersion &lt;string> \* only version 1.0 will be supported for now<br /> - nonce &lt;int><br /> - vaspDomain &lt;string><br /> - signature &lt;string><br /> - isSubjectToTravelRule &lt;bool><br /> - Timestamp &lt;long> | uma-server responds with a bad request response and a proper error message. |   |
...
The provided Lightning Network invoice (shortened in the above example) can then be used to perform a withdrawal or convert withdrawal operation (see [BOLT11 withdrawals guide](/docs/bolt11-deposits-and-withdrawals)).
