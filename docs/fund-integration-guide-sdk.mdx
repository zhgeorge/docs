---
title: "Onboarding API + Fund SDK"
description: "Instantly fund an account anytime, anywhere - end to end integration guide for the Account Funding product"
---

US Integration GuideEU Integration Guide

See core product page [here](/docs/fund-overview)
...
<Info title="Natural vs. Non-natural person">
...
Example request - [POST /participants/customers/new](https://docs.zerohash.com/reference-link/post_participants-customers-new)
...
See more detail on permitted jurisdictions [here](/docs/permitted-and-restricted-jurisdictions).

The preferred approach is for the Platform to not allow End Customers to onboard on *their side* (through a feature flag, for example). However, if a request with a Customer in a blocked jurisdiction does get submitted to zerohash via API, the Platform should fail gracefully and display a descriptive error message on-screen.

##### Non-natural person

<Info title="Partial vs. Full Non-natural person">
...
#### Success cases - receive webhook

See [Account Funding Transaction Updates](https://docs.zerohash.com/reference/fund-transaction-update) for more detail on webhooks.

**Deposit processed**

Upon a successful deposit and immediate and automatic conversion to fiat, the Platform will receive a webhook that looks like this:
...
**Deposit pending settlement**

Large stablecoin deposits over \$500k\* can be subject to longer processing times to complete conversion to fiat. When a large deposit is received, the Platform will receive a webhook that looks like this:
...
```json
{
   ...
   "account_label": "general",
  "success": false,
  "status_reason_code": "DEPOSIT_PENDING_SETTLEMENT",
  "reason": "The deposit has been received and is pending settlement. Once processing is completed, the deposit will be converted to fiat and credited to the customer's account.",
...
*\*To enable large stablecoin deposits over \$500k, contact your zerohash Relationship Manager.*
...
If you're using this product in tandem with our Buy/Sell product, see [Account Funding + Buy/Sell Reconciliation](/docs/account-funding-reconciliation).
...
The Platform will use an external, non-zerohash KYC provider to perform proper verification of the Customer. After a successful verification, pass the results to zerohash via API. Guidelines for onboarding EU participants can be found [here](/docs/eu-participant-creation).
