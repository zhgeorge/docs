---
title: "Rewards Integration Guide"
---
...
A status of 'Completed' for this endpoint means that you successfully post crypto rewards to the participant. For any HTTP errors, you can refer to our [API error handling page](/docs/error-handling).

1. These transactions will settle instantly. The platform can then query [GET /accounts](https://docs.zerohash.com/reference-link/get_accounts) and view the balance for `CUST01`. In this example, the Platform (`PLAT01`), is making the call:
...
To view the current float balance for the rewards account, you can use [GET /accounts](https://docs.zerohash.com/reference-link/get_accounts) endpoint and search for the account matching the following descriptions:
