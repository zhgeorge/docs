...
description: "Platforms who manage a ledger on their end must abide by our conformance requirements"
---

## Context

Within the larger [Account Funding](/docs/fund-overview) product, zerohash offers the ability for Platforms to allow their end customers to make crypto and stablecoin withdrawals. The integration guide for the withdrawal flow is [here](/docs/wallet-link-sdk-integration-guide). This page outlines zerohash's official requirements for Platforms who maintain balances on their side.
...
5. The Platform must keep track of withdrawal states on their end. In order to progress the transaction through the Platform’s state machine, the webhooks must be matched per the instructions in the ["Field matching logic" section](/docs/withdrawals-releasing-funds#field-matching-logic). When building your state machine, you must persist zerohash's unique ID, the `payment_id` as well as any of your own (ie, the `reference_id`).
...
You can view the official [integration guide](/docs/wallet-link-sdk-integration-guide) for and end to end overview
...
As a refresher, when [invoking the Withdraw SDK](/docs/wallet-link-sdk-integration-guide), the Platform must specify the following fields:
...
### 5. zerohash sends back a different payment_id than the one that was sent back in the POST /client_auth_token response
