---
title: "Remittances Integration Guide"
description: "Make cross-border payments instantly, 24/7/365"
---
...
- `date_of_birth`, `address_1`, and `address_2` are optional fields, however, they are *required in high risk jurisdictions.*
- `jurisdiction_code`follows the ISO 3166-2 standard.

<Info>
### We highly recommend submitting the date_of_birth. This reduce the percentage of beneficiaries getting stuck in a pending_approval state. The date_of_birth is an important field to reduce the risk of sending assets to a sanctioned person
</Info>
...
- There is a credit check against the [Float Account](/docs/remittance-integration-guide#1-fund-float-account). The balance on the account must be at least the amount of the submitted `total`
...
- The withdrawal will initially be in an APPROVED `status`
  - See guide [here](/docs/withdrawal-api-response-field-interpretation#has-the-balance-been-deducted-from-the-zero-hash-ledger) for explanation on the `status` and `on_chain_status` fields
- Here are the account changes after a successful transfer:
