/* zerohash docs: the setup wizard (/setup-wizard) and the landing page's hero card and backgrounds.
   ...
   Facts, endpoints and payloads come from the guides in docs/, and each guide section links to its source. */
(function () {
  ...
  const PRODUCTS = [
    ...
    { id: "trade", name: "Buy / Sell Crypto", tag: "Trade", short: "Let customers buy and sell crypto.", plain: "You ask zerohash for a firm price, show it to the customer, and confirm. zerohash handles liquidity, custody and compliance behind the scenes.", src: "buysell" },
  ...
  function buildGuide() {
    ...
    phases.push({ eyebrow: "Phase 1", title: "Get set up", intro: `You'll work with ${R.entity}. Start in <b>Cert</b>, zerohash's sandbox with pretend money, at ${h}. When you're approved for real money, the same calls go to ${R.prod}.`,
      steps: [
        ...
        { title: "Create an API key", html: `<p>In the portal: <b>Administration → API Keys → Add API Key</b>. Save the public key, private key and passphrase right away, because you only see them once. Then tell zerohash which IP addresses you'll call from; keys need two approvals before they work.</p>`, links: [link("generate-api-keys"), link("api-security", "API Security")] },
  ...
  const PRODUCT_PHASE = {
    ...
    trade(a, { link, ref }) {
      ...
      ph.config.push("whose dollars fund a buy, either your float account (you settle daily) or each customer's own balance", "your default markup and how long quotes stay valid");
      return ph;
    },
    payouts(a, { link, ref }) {
      ...
      ph.steps.push({ title: "Register who's paying and fund the float", html: `<p>The payer (usually you) is registered once as a business. Payouts come out of a dollar float you keep with zerohash, and it's pre-funded for you in Cert.</p>`, links: [link("new-payouts-api-integration-guide")] });
    ...
    bank(a, { link, ref }) {
      ...
      ph.steps.push({ title: "Link the customer's bank account", html: `<p>Customers connect their bank through Plaid inside a zerohash screen, with no Plaid contract needed on your side. (If you already use Plaid, you can pass zerohash a processor token instead.)</p>`, links: [link("bank-account-linking")] });
  ...
  function mountFaq() {
    ...
    /* The answer is found and rendered here, in the row, from the passage index this site ships
       as /faq-index.txt. It deliberately does not hand off to Mintlify's assistant: the reader
       gets the quote and its source page in place, with the agent buttons beside it. */
